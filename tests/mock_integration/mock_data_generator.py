...
class MockDataGenerator:
    ...
    Usage::
        ...
        gen = MockDataGenerator('/path/to/tap_onfleet/schemas')
        record = gen.generate_record('administrators', seed=0)
        records = gen.generate_records('workers', count=3)
    """
    ...
    def generate_value(self, field_name, field_schema, seed=0):
        ...
        if type_str == 'array':
            ...
            if items_schema.get('properties'):
                obj = {}
                for fname, fschema in items_schema['properties'].items():
                    obj[fname] = self.generate_value(fname, fschema, seed)
                return [obj]
            return []
        if type_str == 'object':
            ...
            if props:
                return {
                    fname: self.generate_value(fname, fschema, seed)
